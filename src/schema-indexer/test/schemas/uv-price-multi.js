...
class Schema {
  async onInit(web3, data) {
    ...
    console.log('MULTI COIN PRICE WETH--------------------------------------------');
    ...
    const csvHeader = "ts,block,symbol,priceWeth";
    this.csv = CSV("./csv/uv_price_ts_2.csv", csvHeader, 0x1000);
    ///this.csv = CSV("./csv/uv_price_coins_3.csv", csvHeader, 0x1000);
    //this.csv = CSV("./csv/uv_price_weth_usdc.csv", "block,symbol,wethReserve,tokenReserve,price", 0x1000);
  ...
  async onBlock(blockNumber) {
    ...
    for (let coin of this.coins){
      if(blockNumber >= coin.lpCreateBlock){
        await this.onCoinBlock(coin, blockNumber);
      }
    }
  }
  //////////////////////////////////////
  async onCoinBlock(coin, blockNumber) {
    if(coin.failed){
      if(!(blockNumber % 100000 )){
        console.log(blockNumber, coin.symbol, "failed!");
      }
      return;
    }
    // SLOWS DOWN THINGS
    ...
    let row = [tsIso, blockNumber, coin.symbol, priceWeth.toFixed(2)];
    this.csv.addRow(row);
