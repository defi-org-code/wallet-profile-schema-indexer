...
class Schema {
  ...
  async onBlock(blockNumber) {
    if( blockNumber % 10000 == 0){
      console.log(`${new Date()})block ${blockNumber}: | rows in csv:${this.csvWriter.rowCounter}`);
    }  
    ...
    if (events.length > 0) {
      for (const event of events) {
        ...
        if(this.flags.balanceMappingSlot) {
          fromBalance = await this.getBalanceByStorage(from);
          toBalance = await this.getBalanceByStorage(to);
        } else {
